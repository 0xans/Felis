...
impl Database {
    ...
    pub fn load_session(&self) -> Result<Vec<Session>> {
        let conn = self.conn.lock().unwrap();
        ...
        let sessions = stmt.query_map([], |row: &rusqlite::Row| {
            Ok(Session {
                id: row.get(0)?,
                hostname: row.get(1)?,
                username: row.get(2)?,
                ...
                status: serde_json::from_str(&row.get::<_, String>(12)?).unwrap_or(SessionStatus::Active),
            })
    ...
    pub fn update_status(&self, id: &str, status: &SessionStatus) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute("UPDATE sessions SET status = ?1 WHERE id = ?2", params![serde_json::to_string(status)?, id])?;
        Ok(())
    }
